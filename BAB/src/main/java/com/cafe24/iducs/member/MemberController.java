...
import org.springframework.web.servlet.ModelAndView;
	...
	public ModelAndView memberHome(ModelAndView mav, HttpServletRequest request) {
		
		HttpSession session = request.getSession(true);	
		
		String userid = (String)session.getAttribute("id");  
		
		MemberDTO user = dao.selectmember2(userid);
		
		System.out.println(user.getId());
		mav.addObject("pw", user.getPw());
		mav.addObject("status", user.getStatus());
		mav.addObject("img", user.getImg());
		
		mav.setViewName("member/member-home");
		return mav;
	}
}
